require 'melissa-data/server'
require 'rest-client'
MelissaData.env = :test

module MelissaData
  # Methods to help with testing MelissaData APIs
  #
  # @example
  #   describe Echo do
  #     include MelissaData::TestHelper
  #
  ...
  module TestHelper
    ...
    # @return [MelissaData::Server] The executed server
    def server(api, port = 9000, options = {}, &blk)
      op = OptionParser.new

      s = MelissaData::Server.new
      s.logger = mock('log').as_null_object
      s.api = api.new
      s.app = MelissaData::Rack::Builder.build(api, s.api)
